...
void BinomialVisualiser(const int n, const long double p) {//n>0, p>0
	...
	const long double BigGap = (MaxProbability / (TotalBlocks)); //gap size between blocks
	...
	for (int i = 0;i <= TotalBlocks; i++) {
		if (i % 2 == 0) { //display numbers on scale every 2 intervals
			Scale += to_string(BigGap *  (i)).substr(0, NumberOfDP + 2) + SpaceBetweenNumbers;
		}
	}
	Scale += "\n\n";
	...
	for (int x = 0;x <= n;x++) {
		...
		//remaining units 
		long double remaining = px - (TotalFilledBlocks)*BigGap;
